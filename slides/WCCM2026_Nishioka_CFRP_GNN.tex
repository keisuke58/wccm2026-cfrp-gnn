...
% ---------------------------------------------------------------------
\begin{frame}{Prior work \& problem formulation}
...
\begin{column}{0.5\textwidth}
  \centering
  \includegraphics[width=\linewidth]{tdps_layer.png}\\
  {\scriptsize TDPS by insertion layer \& ply angle --- mid-thickness (9--12) best.}
\end{column}
\end{columns}
\end{frame}
...
\note{Representative Layer-10 test cases: for each, the normalized DSPSS input, the ground-truth labels, and the model prediction. The model recovers both the in-plane region and the layer for clear cases; harder cases show adjacent-layer slip.}
\centering
\includegraphics[width=0.88\linewidth]{pred_l10.png}\\[3pt]
{\scriptsize Input (i) $\rightarrow$ ground truth (ii) $\rightarrow$ prediction (iii) for representative Layer-10 cases.}
\end{frame}
