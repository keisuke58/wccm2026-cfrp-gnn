...
| `kojima_real_conformal` | 17/17 | **does the conformal guarantee survive the real shift?** Scale-free FEM shape→tail regression, split-conformal calibrated on FEM, tested on FEM (in-dist) vs the 22 REAL specimens (OOD). FEM-test coverage tracks nominal (91/96/100% @ 80/90/95); **FEM-calibrated coverage on REAL collapses to ~0%** (residuals inflate ×194 — exchangeability broken; **distribution-free ≠ shift-free**); **real leave-one-out recalibration restores nominal** (82/91/96%) → the model transfers, the CALIBRATION must be redone on real. Honest: n=22 wide CI, scalar shape-regression (not the GNN), and the FEM pool is homogeneous so the literal 0% is amplified — the lesson is the DIRECTION |
...
| `qubo_maintenance` | 23/23 | **fleet turnaround schedule as a QUBO** (discrete complement of `cross_structure_design`'s continuous split; **quantum-annealing-ready** = Keio/Muramatsu bridge): pick inspect/repair/fly-as-is per vehicle under NDT-slot, repair-bay & **weighted crew-hour knapsack** caps (inspect=1, repair=3 crew-h); per-vehicle values reuse the framework (inspect=EVSI `voi_inspection`, repair=bay-saving `system_baseline`). **SA solves the QUBO to the EXACT optimum (gap 0.00%** vs brute force); on a crafted knapsack where density-greedy mis-packs, the global solve **provably beats greedy 30 vs 25 (+20%)**; on random fleets greedy is strong (≈0% edge) so the QUBO's worth is the **optimality guarantee + the QA-ready Q matrix**. Honest: representative penalty weights (constraint-satisfaction reported), SA heuristic (gap vs brute force/greedy, no global certificate) |
