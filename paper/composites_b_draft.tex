...
toughening is the dominant lever, and the off-axis angle is rail-bound at the
manufacturable cap, an honest signal that the baseline is not interior-optimal.
\begin{figure}[t]\centering
...
($\sim$100 unit tests) in two public-on-request repositories, including a
one-command end-to-end run (detect$\to$characterise$\to$prognose$\to$clear
$\to$fleet-update in $<$4\,s) and a decision-level UQ harness that scores the
final clearance call against the exact FD oracle; figures are generated by the
