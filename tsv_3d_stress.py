...
Concept demo (single via, linear thermoelastic, structured tets, clamped base): not a
device-accurate packaging run. Shows that real (anisotropic) constants change the KOZ
shape a 2-D isotropic model cannot capture.
...
# ----------------------------------------------------------------------------
def solve(nodes, tets, C_si, aniso_label=""):
    nN = len(nodes)
    ndof = 3 * nN
    P = nodes[tets]                                  # (E,4,3)
    E = len(tets)
    ...
    # clamp the base (z = 0): substrate anchor
    base = nodes[:, 2] < 1e-9
    fixed = np.repeat(base, 3)
    freed = ~fixed
    u = np.zeros(ndof)
    u[freed] = spla.spsolve(K[freed][:, freed].tocsc(), F[freed])
...
def main():
    ...
    n = args.n
    vm_aniso = solve(nodes, tets, c_cubic(C11_SI, C12_SI, C44_SI), "aniso-Si")
    vm_iso = solve(nodes, tets, c_isotropic(E_SI_ISO, NU_SI_ISO), "iso-Si")
